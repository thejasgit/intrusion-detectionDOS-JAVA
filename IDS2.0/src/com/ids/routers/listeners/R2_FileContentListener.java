 ...
 * @ProjectName IDS
 * @FileName R2_FileContentListener.java
 * @DateofCreation Oct 15, 2012
 * @CreatedTime 2:51:16 PM
 */
public class R2_FileContentListener implements Runnable 
{
	int port;
	...
	public R2_FileContentListener(int port) 
	{
		this.port = port;
	}
	...
	public void run() 
	{
		try
		{
			if(this.port == Constants.FILE_REC_R2_SERVER_PORT_NUM) 
   			{
				server = new ServerSocket(Constants.FILE_REC_R2_SERVER_PORT_NUM); 
...
//					 String file = received_data1.trim();//file name to save
					 ...
					 File f1=new File(Constants.R2_STORAGE+fileName);  
