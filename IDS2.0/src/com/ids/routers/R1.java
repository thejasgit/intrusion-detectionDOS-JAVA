...
import com.ids.routers.listeners.R1_HeaderContentListener;
...
public class R1 extends JFrame
{
	...
	static boolean statusCloseFlag = false;
	...
	public R1() throws IOException
	{
		...
		Thread t = new Thread(new R1_MsgListener(Constants.R1_SERVER_PORT_NUM)); 
		t.setName("Listener-" + Constants.R1_SERVER_PORT_NUM);
		t.start(); 
		
		Thread t1 = new Thread(new R1_FileContentListener(Constants.FILE_REC_R1_SERVER_PORT_NUM)); 
		t1.setName("Listener-" + Constants.FILE_REC_R1_SERVER_PORT_NUM);
		t1.start();
		
		Thread t2 = new Thread(new R1_HeaderContentListener(Constants.HEADER_REC_R1_SERVER_PORT_NUM)); 
		t2.setName("Listener-" + Constants.HEADER_REC_R1_SERVER_PORT_NUM);
		t2.start();
		
		Thread t3 = new Thread(new R1_IntruderListener(Constants.INTRUDE_REC_R1_SERVER_PORT_NUM)); 
		t3.setName("Listener-" + Constants.INTRUDE_REC_R1_SERVER_PORT_NUM);
		t3.start();
		
		Thread t4 = new Thread(new R1_IntruderHListener(Constants.INTRUDE_H_REC_R1_SERVER_PORT_NUM)); 
		t4.setName("Listener-" + Constants.INTRUDE_H_REC_R1_SERVER_PORT_NUM);
		t4.start();
...
//			file_lbl.setText("12");
			...
			i_jp =new JPanel();
			i_jp.setBounds(new java.awt.Rectangle(75,180,600,285));
...
//			i_jp.add(getSendJbutton()); 
	...
	public static void sendRecData(String rec_data) throws InterruptedException  
	{
		...
		values = ids.getCurrLodAndMaxTh(profName,Constants.R1);
		SupportCalculation.checkCriteria(values,profName,Constants.R1);
		
		
		
		String updateStatus = ids.getStatus(Constants.R1,profName);
		System.out.println("---------->>> " + updateStatus);
		if(updateStatus.trim().equals(Constants.PROFILE_OPEN))
		{
			ids.updateCurrentLod(profName,Constants.R1);
	...
	public static void sendRecACk(String fileName) 
	{
		...
		try
		{
			...
			if(status.trim().equals(Constants.PROFILE_OPEN))
			{
			    ...
			    String fl_path = Constants.R1_STORAGE+fileName; 
...
//				JOptionPane.showMessageDialog(null, "closed");
	...
	public static void sendRecHeader(String header)
	{
		...
		IdsDAO ids = new IdsDAOImpl();
		int totProfile = 0;
		boolean flag = ids.checkProfileExistence(header,Constants.R1);
		if(!flag)
		{
			ids.insertProfile(header,Constants.R1_LOAD,Constants.MIN_THRESHOLD,Constants.R1,Constants.PROFILE_OPEN); 
			totProfile = ids.getTotalProfiles(Constants.R1);
			int maxThreshold = SupportCalculation.getMaxTh(totProfile,Constants.R1_LOAD); 
			ids.updateMaxThreshold(maxThreshold,Constants.R1);
